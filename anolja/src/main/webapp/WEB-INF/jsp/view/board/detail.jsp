				...
				<tbody>
					<c:forEach var="result" items="${result.list}">
						<tr>
							<td>${result.bNo}</td>
							<td><a
								href="${pageContext.request.contextPath}/board/detail?bNo=${result.bNo}">${result.title}</a></td>
							<td>${result.anonymousId}</td>
	...
	<form action="commentUpdate" method="post">
		...
		<form id="rForm" class="form-inline">
			<div id="comment">
				...
				<div class="form-group">
					<input type="text" name="content" class="form-control input-wo1" style="width:750px; height:50px; padding-left:50px;" placeholder="내용을 입력하세요"  />
				</div>	
