			...
			<div class="content_row_2">
				...
				<table class="board_table">
					...
					<c:forEach var="search" items="${search}">
						<tr>
							<td><a href="${pageContext.request.contextPath}/board/detail?bNo=${search.bNo}&pageNo=${searchpageNo}">${search.bNo}</a></td>
							<td>
								<a href="${pageContext.request.contextPath}/board/detail?bNo=${re.bNo}&pageNo=${search.pageNo}">
									<c:if test="${search.groupBlist > 0}">
										<c:forEach begin="1" end="${search.depth}">
											RE : 
										</c:forEach>
									</c:if>							
								${search.title}
							...
							<td>${search.anonymousId}</td>
							<td><fmt:formatDate value="${search.regDate}" pattern="yyyy-MM-dd" />
							<td>${search.viewCnt}</td>
						</tr>
					</c:forEach>
